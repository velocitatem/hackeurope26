...
export async function GET(request: NextRequest) {
  const jobExternalId = request.nextUrl.searchParams.get("job_external_id");
  const query = jobExternalId
  ...
  try {
    ...
    return NextResponse.json({ schedulingDecisions, migrationEvents });
  } catch (err) {
